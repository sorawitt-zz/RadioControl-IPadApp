...
public struct AnalogJoystickData: CustomStringConvertible {
    ...
    mutating func resetDirectionX() {
        velocity = CGPoint(x: 0.0, y: velocity.y)
        angular = 0
    }
...
open class AnalogJoystickComponent: SKSpriteNode {
    ...
    init(diameter: CGFloat, color: UIColor? = nil, image: UIImage? = nil) {
        super.init(texture: nil, color: color ?? UIColor.black, size: CGSize(width: diameter*2, height: diameter*2))
...
open class AnalogJoystick: SKNode {
    ...
    internal var tracking = false
    internal(set) var data = AnalogJoystickData()
